...
const CartList = ({ items, allItems, handleCount, total }) => {
    return (
        <ul className={styles.cartList}>
            {(items.length < 1) && <p className={styles.empty}>Your cart is empty</p>}
            {items.map(item => {
            ...
            <div className={styles.totals}>
                <div className={styles.lineItem}>
                    ...
                    <div className={`${styles.amount} ${styles.price} ${styles.subtotal}`}>$000</div>
                </div>
                <div className={styles.lineItem}>
                    <div className={styles.label}>Tax:</div>
                    <div className={`${styles.amount} ${styles.price} ${styles.tax}`}>$1.05</div>
                </div>
                <div className={`${styles.lineItem} ${styles.total}`}>
                    <div className={styles.label}>Total:</div>
                    <div className={`${styles.amount} ${styles.price} ${styles.total}`}>$11.85</div>
                </div>
            </div>
        </ul>
    )
